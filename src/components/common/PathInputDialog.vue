...
const visible = computed(() => pathPrompt.value.visible);
const options = computed(() => pathPrompt.value.options);
  ...
  <Teleport to="body">
    <div v-if="visible" class="prompt-overlay" @click.self="cancel">
      <div class="prompt-dialog" role="dialog" aria-modal="true" :aria-label="options.title">
        <h3 class="prompt-dialog__title">
          <i :class="['fas', options.mode === 'directory' ? 'fa-folder-open' : 'fa-file-lines']"></i>
          {{ options.title }}
        </h3>
